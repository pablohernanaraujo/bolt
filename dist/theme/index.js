...
function setTheme(theme, element = document.documentElement) {
...
function toggleTheme(element = document.documentElement) {
    const current = getCurrentTheme(element);
    const next = current === 'light' ? 'dark' : 'light';
    setTheme(next, element);
    return next;
}
function initTheme(element = document.documentElement) {
...
function setBrand(brand, element = document.documentElement) {
    element.setAttribute('data-brand', brand);
}
function getBrand(element = document.documentElement) {
    return element.getAttribute('data-brand');
}

export { getBrand, getCurrentTheme, getThemeScript, initTheme, setBrand, setTheme, toggleTheme, watchSystemTheme };
