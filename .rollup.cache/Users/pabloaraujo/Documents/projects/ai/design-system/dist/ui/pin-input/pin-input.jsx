...
export const PinInput = ({ length = 4, type = 'numeric', variant = 'outline', size = 'medium', hasError = false, disabled = false, autoFocus = false, masked = false, maskChar = '•', value, defaultValue = '', onChange, onComplete, onFocus, onBlur, 'aria-label': ariaLabel, 'aria-describedby': ariaDescribedBy, className, children, }) => {
    ...
    return (<PinInputContext.Provider value={contextValue}>
      <div className={`${containerClassName} ${componentClassName}`} role="group" aria-label={ariaLabel || `PIN input with ${length} digits`} aria-describedby={ariaDescribedBy} data-error={hasError || undefined} data-disabled={disabled || undefined}>
        
        <input ref={hiddenInputRef} type="text" name="pin" defaultValue={getPinValue(currentValues)} className={hiddenInput} tabIndex={-1} aria-hidden="true" readOnly/>
